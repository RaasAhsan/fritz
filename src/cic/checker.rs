...
pub fn typecheck(term: &Term, global: &GlobalEnvironment, local: &LocalContext) -> Term {
    match term {
        ...
        Term::Forall(name, ty, body) => {
            let _ = typecheck(ty, global, local);
            let mut next_local = local.clone();
            if let Some(n) = &name {
                // TODO: we need to beta-reduce the term we are checking
                next_local.declare_assumption(n.clone(), *ty.clone());
            }
            let tty = typecheck(body, global, &next_local);
            if tty == Term::Prop {
                Term::Prop
            } else {
                panic!("Invalid sort")
        ...
        Term::Abs(name, ty, term) => {
            ...
            Term::Forall(name.clone(), ty.clone(), Box::new(tty))
        }
        Term::App(t1, t2) => {
            if let Term::Forall(name, aty, rty) = typecheck(t1, global, local) {
                let t2ty = typecheck(t2, global, local);
                if t2ty == *aty {
                    let mut tty = rty;
                    if let Some(n) = name {
...
mod tests {
    ...
    fn global_type() {
        ...
        assert_eq!(
            typecheck_closed(&constant_term("nat"), &global),
            type_term()
        );
    }
